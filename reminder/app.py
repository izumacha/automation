...
class PlannerApp:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        # 過去時刻が初期値にならないようにする（過去時刻のままだと make_due の
        # 翌日繰り上げ判定により、意図せずタスクが翌日に登録されてしまうため）。
    ...
    def _format_scheduled_message(self, title: str, verb: str, due_dt: datetime.datetime) -> str:
        ...
        表示中の「今日」に載らないタスクはメッセージにも日付を含め、別の日へ登録されたことを
        利用者に明示する。日付を出さないと HH:MM だけでは「今日追加された」と誤解されてしまう。

        判定は暦日ではなく planner_day（起床〜就寝で区切る一日）で行う。ヘッダの「今日」表示・
        デイビューの表示対象と同じ区切りに揃えないと、夜型レンジ（例: 就寝 01:00）の深夜帯で
        メッセージと画面が食い違う: 深夜 00:30 に翌午前のタスクを追加すると暦日は同じなので
        時刻のみ表示されるのに画面（前日のプランナー日）には現れず、逆に 23:30 に翌暦日
        00:15 のタスクを追加すると暦日が違うので日付付きで表示されるのに画面には今日として
        描かれてしまう（両方向の食い違いを planner_day 基準で解消する）。
        """
        wake, sleep = self._wake_min(), self._sleep_min()  # 起床・就寝時刻を「分」で取得する（planner_day の区切りに使う）
        if planner_day(due_dt, wake, sleep) != self._planner_today():  # 開始日時のプランナー日が表示中の「今日」でない場合
    ...
    def _render_backlog(self, today: datetime.date,
        ...
        # 「完了」「予定に追加」ボタンが無反応になる回帰が生じる（カレンダー側の
