...
# 1 分あたりのピクセル数（HOUR_HEIGHT から導出する）。
# 「分 → px」と「px → 分」の換算はカレンダー描画側に 2 か所ある
# （ブロックの y 座標を出す scale と、最低高さを分に直す _min_visual_minutes）。
# 各所で HOUR_HEIGHT / 60 と書き下すと、片方だけを変える改修（例: 表示倍率の導入）で
# 2 つの換算が食い違い、重なっていない隣接タスクが「重なっている」と判定されて
# カードが半幅に割れる。導出をここ 1 か所に置き、両方がこの値を参照する（§6）
