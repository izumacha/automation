...
# 1 分あたりのピクセル数（HOUR_HEIGHT から導出する）。倍率を掛けない素の値。
# 唯一の消費者は PlannerApp._px_per_minute() で、描画の scale と
# _min_visual_minutes()（px→分の逆換算）はどちらもそのメソッドを経由する。
# **表示倍率（ズーム）を入れるならこの定数ではなく _px_per_minute() の中で掛ける**:
# 倍率はウィンドウ/インスタンスごとに変わりうるのでモジュール定数には載せられず、
# ここに載せると描画側だけが倍率に追随して 2 つの換算が食い違い、
# 重なっていない隣接タスクが「重なっている」と判定されてカードが半幅に割れる
PX_PER_MINUTE = HOUR_HEIGHT / 60.0
